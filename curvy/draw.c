...
void add_circle( struct matrix * points, 
  ...
  for (i=0.0;i<1.0;i+=step){
    add_edge(points,cx+r*cos(2*M_PI*i),cy+r*sin(2*M_PI*i),0,
	     cx+r*cos(2*M_PI*(i+step)),cy+r*sin(2*M_PI*(i+step)),0);
  }
}
...
void add_curve( struct matrix *points, 
  ...
  for (i=0.0;i<1.0;i+=step){
    ...
    x = coefx->m[0][0]*pow(i,3)+coefx->m[1][0]*pow(i,2)+coefx->m[2][0]*i+coefx->m[3][0];
    y = coefy->m[0][0]*pow(i,3)+coefy->m[1][0]*pow(i,2)+coefy->m[2][0]*i+coefy->m[3][0];
    add_point(points,x,y,0);
  }
  free(coefx);
  free(coefy);
