...
struct matrix * make_bezier() {
  ...
  bezier->m[0][0]=-1;
  bezier->m[0][3]=bezier->m[3][0]=1;
  bezier->m[0][1]=bezier->m[1][0]=bezier->m[1][2]=bezier->m[2][1]=3;
  bezier->m[1][1]=-6;
  bezier->m[2][0]=bezier->m[0][2]=-3;
  return bezier;
}
...
struct matrix * make_hermite() {
  ...
  hermite->m[0][2]=hermite->m[0][3]=hermite->m[2][2]=hermite->m[3][0]=1;
  hermite->m[1][3]=-1;
  hermite->m[0][1]=hermite->m[1][2]=-2;
  hermite->m[1][0]=-3;
  hermite->m[0][0]=2;
  hermite->m[1][1]=3;
  return hermite;
...
struct matrix * generate_curve_coefs( double p1, double p2, 
  ...
  struct matrix* coeff_gen;
  if (type == BEZIER_MODE){//bezier
    coeff_gen = make_bezier();
  }
  else if (type == HERMITE_MODE){//hermite
    coeff_gen = make_hermite();
  }
  struct matrix* coefficients = new_matrix(4,1);
  coefficients->m[0][0]=p1;
  coefficients->m[1][0]=p2;
  coefficients->m[2][0]=p3;
  coefficients->m[3][0]=p4;
  matrix_mult(coeff_gen,coefficients);
