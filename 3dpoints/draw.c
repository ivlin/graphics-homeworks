...
void generate_sphere( struct matrix * points, 
		      double cx, double cy, double r, 
		      double step ) {
  double create, rotate;
  double x,y,z;
  for (rotate=0.0;rotate<1.0;rotate+=step){
    for (create=0.0;create<1.0;create+=step){
      x=cx+r*cos(M_PI*create);
      y=cy+r*sin(M_PI*create)*cos(2*M_PI*rotate);
      z=r*sin(M_PI*create)*sin(2*M_PI*rotate);
...
void generate_torus( struct matrix * points, 
		     double cx, double cy, double r1, double r2, 
		     double step ) {
  //r1 will be radius tiny r2 will be radius big
  double create, rotate;
  double x,y,z;
  for (rotate=0.0;rotate<1.0;rotate+=step){
    for (create=0.0;create<1.0;create+=step){
      x=cx+r1*cos(M_PI*create);
      y=cy+(r1*sin(M_PI*create)+r2)*cos(2*M_PI*rotate);
      z=(r1*sin(M_PI*create)+r2)*sin(2*M_PI*rotate);
      add_point(points,x,y,z);
      add_point(points,x,y,z);
    }
  }
