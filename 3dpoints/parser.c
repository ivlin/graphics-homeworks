...
void parse_file ( char * filename, 
  ...
  while ( fgets(line, 255, f) != NULL ) {
    ...
    else if ( strncmp(line, "box", strlen(line)) == 0 ) {//cxcycz lwh
      fgets(line, 255, f);
      sscanf(line, "%lf %lf %lf %lf %lf %lf",
	     &x1, &y1, &z1, &wid, &hi, &dep);
      add_box(pm,x1,y1,z1,wid,hi,dep);
    }
    else if ( strncmp(line, "sphere", strlen(line)) == 0 ) {//cx cy rad
      fgets(line, 255, f);
      sscanf(line, "%lf %lf %lf",
	     &x1, &y1, &r1);
      generate_sphere(pm,x1,y1,r1,DEFAULT_STEP);
    }
    else if ( strncmp(line, "torus", strlen(line)) == 0 ) {//cx cy smallrad bigrad
      fgets(line, 255, f);
      sscanf(line, "%lf %lf %lf %lf",
	     &x1, &y1, &r1, &r2);
      generate_torus(pm,x1,y1,r1,r2,DEFAULT_STEP);
    }
