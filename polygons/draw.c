...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  double normx,normy,normz,viewx,viewy,viewz;
  viewx=viewy=0.0;
  viewz=-1.0;
  double costheta;
  int i;
  ...
  for (i=0;i<polygons->lastcol-2;i+=3){
    x0=polygons->m[0][i];x1=polygons->m[0][i+1];x2=polygons->m[0][i+2];
    y0=polygons->m[1][i];y1=polygons->m[1][i+1];y2=polygons->m[1][i+2];
    z0=polygons->m[2][i];z1=polygons->m[2][i+1];z2=polygons->m[2][i+2];
    normx=(y1-y0)*(z2-z0)-(z1-z0)*(y2-y0);
    normy=(z1-z0)*(x2-x0)-(x1-x0)*(z2-z0);
    normz=(x1-x0)*(y2-y0)-(y1-y0)*(x2-x0);
    costheta = (normx*viewx+normy*viewy+normz*viewz)/(pow(normx*normx+normy*normy+normz*normz,0.5)*pow(viewx*viewx+viewy*viewy+viewz*viewz,0.5));
...
void add_sphere( struct matrix * points, 
  ...
  for ( lat = 0; lat < num_steps; lat++ ) {
    for ( longt = 0; longt < num_steps/2;longt++ ) {
      index = lat * num_steps + longt;
      index2 = (index + 1)%temp->lastcol;
      index3 = (index + num_steps + 1)%temp->lastcol;
      index4 = (index + num_steps)%temp->lastcol;
      ...
      add_polygon(points,temp->m[0][index],temp->m[1][index],temp->m[2][index],
		  temp->m[0][index3],temp->m[1][index3],temp->m[2][index3],
		  temp->m[0][index4],temp->m[1][index4],temp->m[2][index4]);    
...
void add_box( struct matrix * points,
  ...
  y2 = y - height;
  z2 = z - depth;
  add_polygon(points,x,y,z,x,y2,z,x2,y2,z);//front-lower-left
  add_polygon(points,x,y,z,x2,y2,z,x2,y,z);//front-upper-right
  add_polygon(points,x2,y,z,x2,y2,z,x2,y2,z2);//right-lower-left
  add_polygon(points,x2,y,z,x2,y2,z2,x2,y,z2);//right-upper-right
  add_polygon(points,x2,y,z2,x2,y2,z2,x,y2,z2);//back-lower-left
  add_polygon(points,x2,y,z2,x,y2,z2,x,y,z2);//back-lower-left
  add_polygon(points,x,y,z2,x,y2,z2,x,y2,z);//left-lower-left
  add_polygon(points,x,y,z2,x,y2,z,x,y,z);//left-upper-right
  add_polygon(points,x,y,z,x2,y,z,x2,y,z2);//top-lower-left
  add_polygon(points,x,y,z,x2,y,z2,x,y,z2);//top-upper-right
  add_polygon(points,x,y2,z,x2,y2,z2,x2,y2,z);//bottom-lower-left
  add_polygon(points,x,y2,z,x,y2,z2,x2,y2,z2);//bottom-upper-right
}
